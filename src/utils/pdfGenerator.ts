...
export const generatePDFReceipt = (data: ApplicationData) => {
  const doc = new jsPDF()
  ...
  doc.text('GAZA DAMAGE ASSESSMENT SYSTEM', pageWidth / 2, yPos, { align: 'center' })
  yPos += 8

  doc.setFontSize(12)
  doc.setFont('helvetica', 'normal')
  doc.text('Ministry of Public Works and Housing', pageWidth / 2, yPos, { align: 'center' })
  yPos += 8
  doc.text('Official Application Receipt', pageWidth / 2, yPos, { align: 'center' })
  yPos += 15
  ...
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`Address Before War: ${data.addressBeforeWar || 'N/A'}`, margin, yPos)
  yPos += 6
  doc.text(`Property Type: ${data.propertyType?.toUpperCase() || 'N/A'}`, margin, yPos)
  yPos += 6
  ...
  yPos += 6
  doc.text(`Damage Level: ${data.damageLevel?.toUpperCase() || 'N/A'}`, margin, yPos)
  yPos += 6
  doc.text(`Currently Inhabitable: ${data.isInhabitable ? 'No' : 'Yes'}`, margin, yPos)
  yPos += 10
