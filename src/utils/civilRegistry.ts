...
export interface CivilRegistryData {
  nationalId: string
  fullName: string
  motherName: string
  dateOfBirth: string
...
export type VerificationQuestionType = 'wifeBirthday' | 'sonBirthday' | 'numberOfSons' | 'numberOfDaughters' | 'daughterBirthday' | 'fatherName' | 'occupation' | 'maritalStatus' | 'phoneNumber' | 'motherName'
...
export const verificationQuestions: VerificationQuestion[] = [
  {
    ...
    validate: (answer, data) => {
      const answerDate = answer.replace(/\//g, '-').split('-').reverse().join('-')
      return answerDate === data.wifeDateOfBirth || answer === data.wifeDateOfBirth
    },
  ...
  },
  {
    ...
    validate: (answer, data) => {
      ...
      const answerDate = answer.replace(/\//g, '-').split('-').reverse().join('-')
      return answerDate === data.sonsBirthdays[0] || answer === data.sonsBirthdays[0]
    },
  ...
  },
  {
    ...
    validate: (answer, data) => {
      ...
      const answerDate = answer.replace(/\//g, '-').split('-').reverse().join('-')
      return answerDate === data.sonsBirthdays[1] || answer === data.sonsBirthdays[1]
    },
  ...
  },
  {
    ...
    validate: (answer, data) => {
      ...
      const answerDate = answer.replace(/\//g, '-').split('-').reverse().join('-')
      return answerDate === data.daughtersBirthdays[0] || answer === data.daughtersBirthdays[0]
    },
  ...
  },
  {
    ...
    validate: (answer, data) => answer.toLowerCase().trim() === data.fatherName.toLowerCase().trim(),
  ...
  },
  {
    ...
    validate: (answer, data) => answer.toLowerCase().trim() === data.occupation.toLowerCase().trim(),
  ...
  },
  {
    ...
    validate: (answer, data) => answer.toLowerCase().trim() === data.maritalStatus.toLowerCase().trim(),
  ...
  },
  {
    ...
    validate: (answer, data) => answer.replace(/\s/g, '') === data.phoneNumber.replace(/\s/g, ''),
  ...
  },
  {
    ...
    validate: (answer, data) => answer.toLowerCase().trim() === data.motherName.toLowerCase().trim(),
    getAnswer: (data) => data.motherName,
  },
]

// Get random questions (3 out of available questions)
export const getRandomQuestions = (nationalId: string): VerificationQuestion[] => {
  ...
  const applicableQuestions = verificationQuestions.filter((q) => {
    ...
    if (q.id === 'daughterBirthday1' && data.numberOfDaughters === 0) return false
    return true
  })
