...
import DamageAssessmentStepper from "../Shared/DamageAssessmentStepper";
import {
  Button,
  CircularProgress,
  DialogActions,
    ...
    <div className={classNames("w-full mx-auto", { "p-4 sm:p-8": isPage })}>
      ...
      >
        ...
        <form className="space-y-6">
          ...
          {!readOnly && (
            <DialogActions sx={{ mt: 4, px: 0, justifyContent: "space-between" }}>
              <Box>
                <Button
                  size="large"
                  onClick={() => {
                    ...
                    if (currentValues.buildingType) {
                      const dataToSync = buildFormDataWithoutImages(currentValues);
                      dispatchByType(dispatch, currentValues.buildingType, dataToSync);
                    }
                    if (isChangeToReviewPage) {
