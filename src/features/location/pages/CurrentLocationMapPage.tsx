...
const CurrentLocationMapPage = () => {
    ...
    if (initialLoc) {
      if (!selectedGovernorateId) {
        setSelectedGovernorateId(
          (
            initialLoc.governorate_id || initialLoc.governorate?.id
          )?.toString() || "",
        );
        setSelectedMunicipalityId(
          (
            initialLoc.municipality_id || initialLoc.municipality?.id
          )?.toString() || "",
        );
        setSelectedNeighborhoodId(
          (
            initialLoc.neighborhood_id || initialLoc.neighborhood?.id
          )?.toString() || "",
        );
  ...
  useEffect(() => {
    if (
      position &&
      (localLmData.length > 0 || localGovData.length > 0) &&
        ...
        <CardContent sx={{ p: { xs: 2, sm: 4 } }}>
          ...
          >
            ...
            <Button
              ...
              disabled={loading || (!isInsideGaza && !outsideAddress.trim())}
              startIcon={
                !loading && (
                  <Check
                    className={language === "ar" ? "ml-2" : "mr-2"}
