  ...
  async (payload: IAuthState, { rejectWithValue }) => {
    if (payload.pathSignUp === API.citizen.auth.completeSignup) {
      ...
      try {
        ...
        if (payload.grandfatherName) requestBody.grandfatherName = payload.grandfatherName;
        if (payload.familyName) requestBody.familyName = payload.familyName;
        if (payload.email) requestBody.email = payload.email;
        if (payload.familyMembersNumber) requestBody.familyMembersNumber = payload.familyMembersNumber;
        if (payload.phoneNumber) requestBody.phoneNumber = payload.phoneNumber;
        if (payload.whatsappNumber) requestBody.whatsappNumber = payload.whatsappNumber;

        const res = await axiosClient.post(`${payload.pathSignUp}`, requestBody);
        const token = res.data?.token;
        console.log(res.data);
