...
const FamilyInfoPage = () => {
  ...
  const { t } = useLanguage()
  const { data, setFamilyInfo } = useApplicationStore()
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    defaultValues: {
      addressBeforeWar: data.addressBeforeWar || '',
      numberOfChildren: data.numberOfChildren || 0,
      wifeName: data.wifeName || '',
      wifeNationalId: data.wifeNationalId || '',
      phoneNumber: data.phoneNumber || '',
    }
  })

  const onSubmit = (formData: FormData) => {
    setFamilyInfo(formData)
    ...
    <div className="max-w-2xl mx-auto">
      <div className="card">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            <label htmlFor="addressBeforeWar" className="block text-sm font-medium text-gray-700 mb-2">
              {t('form.addressBeforeWar')} <span className="text-red-500">*</span>
            </label>
            <textarea
              id="addressBeforeWar"
            ...
            {errors.addressBeforeWar && (
              <p className="mt-1 text-sm text-red-600">{errors.addressBeforeWar.message}</p>
            )}
          </div>

          <div>
            <label htmlFor="numberOfChildren" className="block text-sm font-medium text-gray-700 mb-2">
              {t('form.numberOfChildren')} <span className="text-red-500">*</span>
            </label>
            <input
              id="numberOfChildren"
              type="number"
          ...
          <div>
            ...
            />
            {errors.wifeName && (
              <p className="mt-1 text-sm text-red-600">{errors.wifeName.message}</p>
            )}
          </div>

          <div>
            <label htmlFor="wifeNationalId" className="block text-sm font-medium text-gray-700 mb-2">
            ...
            {errors.wifeNationalId && (
              <p className="mt-1 text-sm text-red-600">{errors.wifeNationalId.message}</p>
            )}
          </div>
          ...
          <div className="flex gap-4">
            <button type="button" onClick={() => navigate('/personal-info')} className="btn-outline flex-1">
              {t('common.back')}
            </button>
