...
const PreviousLocationMapPage = () => {
    ...
    const loadData = async () => {
      try {
        ...
        if (govBackendRes.data.governorates)
          setGovernoratesList(govBackendRes.data.governorates);
      } catch (err) {
  ...
  useEffect(() => {
    if (selectedGovernorate) {
      const gov = findMatch(
        governoratesList,
        GOV_MAPPING[selectedGovernorate] || selectedGovernorate,
      );
      console.log(gov.id);
      if (gov) {
        setMuniLoading(true);
      ...
      if (muni) {
        ...
        axiosClient
          .get(API.locations.neighborhoods, {
            params: { municipality_id: muni.id },
          })
          .then((res: any) => {
  ...
  useEffect(() => {
    if (selectedNeighborhoodName && allNeighborhoods.length > 0) {
      ...
      if (nh) {
        setLmLoading(true);
        axiosClient
          .get(API.locations.landmarks, { params: { neighborhood_id: nh.id } })
          .then((res: any) => {
  ...
  const handleGovernorateChange = (event: any) => {
    ...
    if (name) {
      const govLandmarks = localLmData.filter(
        (f) =>
          f.properties.المحافظة === name ||
          f.properties.Governorat === name ||
  ...
  const handleMunicipalityChange = (event: any) => {
    ...
    if (name) {
      ...
      const muniFeature = localMuniData.find(
        (f) =>
          (f.properties.Mun_Name === name ||
            f.properties.Name === name ||
            f.properties.البلدية === name) &&
          (normalizeText(f.properties.المحافظة) ===
            normalizeText(selectedGovernorate) ||
            normalizeText(f.properties.Governorat) ===
              normalizeText(selectedGovernorate) ||
            normalizeText(f.properties.المحا) ===
              normalizeText(selectedGovernorate)),
      );
  ...
  const handleNeighborhoodChange = (event: any) => {
    ...
    const nhFeature = localNhData.find(
      (f) =>
        (f.properties.الحي === name ||
          f.properties.Neighborho === name ||
          f.properties.name === name) &&
        (normalizeText(f.properties.البلدية) ===
          normalizeText(selectedMunicipality) ||
          normalizeText(f.properties.Municipali) ===
            normalizeText(selectedMunicipality) ||
          normalizeText(f.properties.البلد) ===
            normalizeText(selectedMunicipality)),
    );
  ...
  const handleLandmarkChange = (event: any) => {
    ...
    const landmarkFeature = localLmData.find(
      (f) =>
        (f.properties.اسم_المعلم === name ||
          f.properties.name === name ||
          f.properties.Landmark === name) &&
        (normalizeText(f.properties.الحي) ===
          normalizeText(selectedNeighborhoodName) ||
          normalizeText(f.properties.Neighborho) ===
            normalizeText(selectedNeighborhoodName)) &&
        (normalizeText(f.properties.البلدية) ===
          normalizeText(selectedMunicipality) ||
          normalizeText(f.properties.Municipali) ===
    ...
    if (landmarkFeature && landmarkFeature.geometry) {
      const coords: [number, number] = [
        landmarkFeature.geometry.coordinates[1],
        landmarkFeature.geometry.coordinates[0],
      ];
      setCenter(coords);
      setPosition(coords);
      setZoom(18);
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  ...
                  {governorates.map((gov) => (
                    <MenuItem key={gov} value={gov} dir={isRTL ? "rtl" : "ltr"}>
                      {gov}
                    </MenuItem>
                  ))}
                </Select>
              ...
              <FormControl fullWidth size="small">
                ...
                >
                  ...
                  {neighborhoods.map((nh) => (
                    <MenuItem key={nh} value={nh} dir={isRTL ? "rtl" : "ltr"}>
                      {nh}
                    </MenuItem>
                  ))}
                </Select>
