  ...
  const onSubmit = async (data: FormData) => {
    ...
    setTimeout(() => {
      setApplication({
        ...
        lastUpdate: '2024-11-23T14:00:00Z',
        statusHistory: [
          { status: 'submitted', timestamp: '2024-11-23T10:30:00Z' },
          { status: 'underReview', timestamp: '2024-11-23T14:00:00Z' },
        ]
      })
      setLoading(false)
    }, 1000)
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mb-6">
          <div>
            <label htmlFor="trackingNumber" className="block text-sm font-medium text-gray-700 mb-2">
              {t('success.trackingNumber')} <span className="text-red-500">*</span>
            </label>
            <input
              id="trackingNumber"
              type="text"
              {...register('trackingNumber', {
                ...
                pattern: {
                  value: /^GZA-\d{4}-\d{6}$/,
                  message: 'Invalid tracking number format (GZA-YYYY-XXXXXX)'
                }
              })}
              placeholder="GZA-2024-123456"
          ...
          <div className="mt-8 space-y-6">
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <p className="text-sm text-gray-600">
                Tracking Number: <span className="font-mono font-bold">{application.trackingNumber}</span>
              </p>
              <p className="text-sm text-gray-600 mt-2">
                Last Updated: {new Date(application.lastUpdate).toLocaleString()}
              </p>
            </div>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {index === application.statusHistory.length - 1 && (
                          <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded">Current</span>
                        )}
                      </div>
                      <p className="text-sm text-gray-600">
