...
const CurrentLocationMapPage = () => {
  ...
  const { t } = useLanguage()
  const { data, setCurrentLocation } = useApplicationStore()
  const [position, setPosition] = useState<[number, number] | null>(
    data.currentLatitude && data.currentLongitude 
      ? [data.currentLatitude, data.currentLongitude] 
      : null
  )
  const [address, setAddress] = useState(data.currentLocationAddress || '')
  ...
  const handleConfirm = () => {
    if (position) {
      setCurrentLocation(position[0], position[1], address)
      navigate('/review')
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="card">
        <h2 className="text-2xl font-bold mb-2">Select Your Current Location</h2>
        <p className="text-gray-600 mb-4">
          Please click on the map to mark your current location (where you are now).
        </p>
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() => navigate('/damage-assessment-dialog')}
